...
namespace Slip::Reflect {
    ...
    template <typename T>
    struct TypeOf {
        static inline const Type* value() { return T::staticType(); }
    };
    template <typename T>
    struct TypeOf<T*> {
        static inline const Type* value() {
            static const Type t{
                Kind::Pointer, nullptr, TypeOf<T>::value(), "T*", sizeof( T* ), nullptr, {},
            };
            return &t;
        }
    };
    ...
    namespace Detail {
        ...
        template <typename T>
        const Type* dynamicType( const void* addr ) {
            return static_cast<const AbstractReflected*>( addr )->dynamicType();
        }
    }  // namespace Detail
}  // namespace Slip::Reflect
...
#define REFLECT_BEGIN( NAME )                                \
    ...
    Reflect::Type NAME::_Auto::make() {                      \
        Reflect::Type self{};                                \
        typedef NAME DECL;                                   \
        self.name = #NAME;                                   \
        self.kind = Reflect::Kind::Record;                   \
